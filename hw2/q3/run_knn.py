...
def run_knn():
    ...
    for i in k:
        valid_labels = knn(i, train_inputs, train_targets, valid_inputs)
        correct = 0
        for j in range(len(valid_labels)):
            if valid_labels[j] == valid_targets[j]:
                correct += 1
        rate = correct/len(valid_labels)
        prediction_rate.append(rate)
        
    plt.plot(k, prediction_rate)
    plt.xlabel("k")
    plt.ylabel("correct prediction rate")
    plt.title("Correct Prediction Rate of kNN classifier as a function of Hyperparameter k")
    plt.show()
