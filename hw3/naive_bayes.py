...
def train_mle_estimator(train_images, train_labels):
    ...
    for i in range(1):
        pixel_class = train_images[np.nonzero(train_labels[:,i])]
        theta_c = np.sum(pixel_class, axis=0)/len(pixel_class)
        theta_mle[:,i] = theta_c 
...
def train_map_estimator(train_images, train_labels):
    ...
    for i in range(10):
        pixel_class = train_images[np.nonzero(train_labels[:,i])]
        theta_c = (np.sum(pixel_class, 0) + 2)/(len(pixel_class) + 4)
        theta_map[:,i] = theta_c
    
    pi_map = np.sum(train_labels, axis=0)/N  #double check if == mle
...
def log_likelihood(images, theta, pi):
    ...
    for c in range(10):
        log_c = np.log(pi[c]) +\
            np.sum((images * np.log(theta[:,c]) + (1 - images) *\
                np.log(1 - theta[:,c])), axis=1)
        log_like[:, c] = log_c
    # Normalize
    normalized = log_like/np.sum(log_like, axis=1, keepdims=True)
    print(normalized[0])
    return normalized
...
def accuracy(log_like, labels):
    ...
    acc = np.sum(np.equal(labels_index, predict(log_like)))/len(log_like)
    return acc
